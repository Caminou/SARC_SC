...
# SARC data and split into Tumor and PDSC
## 1) Run scDblFinder
```{r}
...
run_doubletfinder_custom <- function(seu_sample_subset, multiplet_rate = NULL){
  ...
  # Pre-process seurat object with standard seurat workflow ---> due to SoupXout data is already SCT normalized and PCA has been calculated :)
  ...
  # run DoubletFinder
print(paste0("Checking doublets from Sample: ", unique(seu_sample_subset[['orig.ident']]), '...........')) 
  sample <- doubletFinder(seu = sample, 
                          ...
                          nExp = nExp.poi.adj, sct=TRUE) # number of expected real doublets
...
samp_DoubletFinder= lapply(SARC_f_list,run_doubletfinder_custom)
sglt_dblt_metadata <- data.frame(bind_rows(samp_DoubletFinder)) # merge to a single dataframe
...
write.table(sglt_dblt_metadata, "home/caminors/Desktop/sc_RNAseq/QC_seurats/SARC_Tumor_PDSC_DoubletFinder.csv")
```
...
## 2) Run DoubletFinder
```{r}
Gruel_DoubletFinder= lapply(Gruel_f_list,run_doubletfinder_custom)
Gruel_sglt_dblt_metadata <- data.frame(bind_rows(Gruel_DoubletFinder)) # merge to a single dataframe
rownames(Gruel_sglt_dblt_metadata) <- Gruel_sglt_dblt_metadata$row_names # assign cell IDs to row names to ensure match
Gruel_sglt_dblt_metadata$row_names <- NULL
head(Gruel_sglt_dblt_metadata)
```
```{r}
...
write.table(Gruel_sglt_dblt_metadata, "/home/caminors/Desktop/sc_RNAseq/QC_seurats/Gruel_DoubletFinder.csv")
```
